...
def sorvete(quantidades: dict, recursos: dict):
    ...
    problema = LpProblem("Sorvete", LpMaximize)

    problema += criar_restricao(vars, quantidades['lucro'], 'objetivo')

    for rec in recursos:
        problema += criar_restricao(vars, quantidades[rec], tipo='<=', valor=recursos[rec])
